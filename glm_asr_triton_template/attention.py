...
# ============================================================================
# ADDITIONAL: FlashAttention kernel — not from TODO, added as optimization #3
# (assignment requirement: "FlashAttention-style attention").
# Reason: The 3-kernel path (scores + softmax + output) materializes a full
# (batch*heads, seq_q, seq_k) attention matrix in GPU memory. This fused
# kernel uses online softmax — it streams through K/V in tiles, never
# allocating the full N×N matrix. This saves memory and reduces kernel
# launch overhead, especially during autoregressive decoding.
# ============================================================================
...
def scaled_dot_product_attention(
    ...
    # ========================================================================
    # ADDITIONAL: Flash attention dispatch path — not from TODO template.
    # Reason: Avoids materializing the full (batch*heads, seq_q, seq_k)
    # attention matrix. Uses online softmax to stream through K/V in tiles.
    # This is assignment optimization requirement #3 (FlashAttention-style).
    # ========================================================================
    can_use_flash = (
        q.is_cuda
    ...
    # ========================================================================
    # ADDITIONAL: Torch/cuBLAS fallback path — not from TODO template.
    # Reason: Use PyTorch SDPA when available (encoder prefill with large
    # seq_k) for better performance than manual einsum; manual fallback
    # for causal/masked cases that can't use SDPA.
    # ========================================================================
    if (
        not is_causal
        and attention_mask is None
