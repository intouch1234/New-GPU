...
def attention_scores_kernel(
    ...
    *** TODO: Implement this kernel ***
    ...
    # Loading query vector for this position
    q = tl.load(
        q_ptr + pid_bh * stride_q0 + pid_q * stride_q1 + offs_d * stride_q2,
        mask=offs_d < head_dim,
        other=0.0,
    )
    # Loading all keys for this batch_head
    k = tl.load(
        k_ptr
        + pid_bh * stride_k0
    ...
    # Compute dot-product scores and scale
...
# ============================================================================
# ADDITIONAL: FlashAttention Kernel — Online Softmax (Streaming)
# Required because scaled_dot_product_attention dispatches to flash_attention_kernel
# for the flash path (Optimization 3: FlashAttention-style fused attention).
# This avoids materializing the full N×N attention matrix, reducing memory from
# O(N^2) to O(N) and improving throughput via tiled K/V streaming.
# ============================================================================

@triton.autotune(
    configs=[
        triton.Config({'TILE_KV': 32}, num_warps=4),
        triton.Config({'TILE_KV': 64}, num_warps=4),
        triton.Config({'TILE_KV': 128}, num_warps=8),
    ],
    key=['seq_k', 'head_dim'],
)
@triton.jit
