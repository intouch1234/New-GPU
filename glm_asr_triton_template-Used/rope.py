...
def apply_rope_kernel(
    ...
    half_dim,
    head_dim,
    stride_x_row,    # stride over (batch*heads*seq) dimension
    stride_cos_row,  # stride over seq dimension for cos/sin
    ...
    Each program handles one position vector of length head_dim.

    For each position:
      x1_rot = x1 * cos - x2 * sin
      x2_rot = x2 * cos + x1 * sin
      pass-through for dims beyond 2*half_dim
    """
    pid = tl.program_id(0)

    # Determine which sequence position this program corresponds to
    # pid = batch_head_idx * seq_len + seq_idx
    seq_idx = pid % seq_len

    offs = tl.arange(0, BLOCK_SIZE)
    mask_half = offs < half_dim

    # Load x1 (first half_dim elements) and x2 (next half_dim elements)
    x_base = pid * stride_x_row
    x1 = tl.load(x_ptr + x_base + offs, mask=mask_half, other=0.0)
    x2 = tl.load(x_ptr + x_base + half_dim + offs, mask=mask_half, other=0.0)

    # Load cos and sin for this sequence position (shape: [seq_len, half_dim])
    cs_base = seq_idx * stride_cos_row
    cos_val = tl.load(cos_ptr + cs_base + offs, mask=mask_half, other=0.0)
    sin_val = tl.load(sin_ptr + cs_base + offs, mask=mask_half, other=0.0)

    # Rotary transform
    x1_rot = x1 * cos_val - x2 * sin_val
    x2_rot = x2 * cos_val + x1 * sin_val

    # Store rotated values
    out_base = pid * stride_x_row
    tl.store(out_ptr + out_base + offs, x1_rot, mask=mask_half)
    tl.store(out_ptr + out_base + half_dim + offs, x2_rot, mask=mask_half)

    # Pass-through for remaining dims beyond 2*half_dim
    rotary_end = half_dim * 2
    pass_offs = tl.arange(0, BLOCK_SIZE)
    mask_pass = pass_offs < (head_dim - rotary_end)
    if head_dim > rotary_end:
        x_pass = tl.load(
            x_ptr + x_base + rotary_end + pass_offs,
            mask=mask_pass,
            other=0.0,
        )
        tl.store(
            out_ptr + out_base + rotary_end + pass_offs,
            x_pass,
            mask=mask_pass,
        )
...
def _apply_rope_single(
    ...
    """Apply RoPE to a single tensor (Q or K) using Triton on CUDA, Torch on CPU."""
    ...
    if x.is_cuda:
        # Triton fused RoPE kernel path
        x_flat = x.to(torch.float32).contiguous()
        total_rows = batch * num_heads * seq_len
        # Reshape to (total_rows, head_dim) for kernel
        x_2d = x_flat.reshape(total_rows, head_dim)
        out_2d = torch.empty_like(x_2d)

        cos_contig = cos.contiguous()
        sin_contig = sin.contiguous()

        block_size = triton.next_power_of_2(max(half_dim, head_dim - half_dim * 2))
        grid = (total_rows,)
        ...
        return out_2d.reshape(batch, num_heads, seq_len, head_dim).to(x.dtype)

    # CPU fallback: element-wise PyTorch ops
    x1 = x[..., :half_dim]
